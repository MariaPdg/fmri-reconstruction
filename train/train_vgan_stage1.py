...
if __name__ == "__main__":
    ...
    saving_dir = os.path.join(SAVE_PATH, 'saved_results', 'gan_{}'.format(timestep))
    ...
    if args.pretrained_gan is not None and os.path.exists(pretrained_model_dir.replace(".pth", ".csv")):
        ...
        if gan_cfg.evaluate:
            ...
            pcc, ssim, mse = evaluate(model, dataloader_valid, norm=True, mean=gan_cfg.mean, std=gan_cfg.std,
                                               path=images_dir)
            print("Mean PCC:", pcc)
